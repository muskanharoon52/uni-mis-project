...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $stmt = $db->prepare('DELETE FROM student_answers WHERE student_answer_id = :id');
    ...
    if (!empty($_POST['student_answer_id'])) {
        ...
        $stmt = $db->prepare('UPDATE student_answers SET student_exam_id = :student_exam_id, question_id = :question_id, question_order = :question_order, question_snapshot = :question_snapshot, selected_option = :selected_option, answered_at = :answered_at, is_correct = :is_correct, marks_awarded = :marks_awarded WHERE student_answer_id = :student_answer_id');
        $stmt->execute($payload);
        $_SESSION['message'] = 'Answer updated successfully.';
    } else {
        $stmt = $db->prepare('INSERT INTO student_answers (student_exam_id, question_id, question_order, question_snapshot, selected_option, answered_at, is_correct, marks_awarded) VALUES (:student_exam_id, :question_id, :question_order, :question_snapshot, :selected_option, :answered_at, :is_correct, :marks_awarded)');
        $stmt->execute($payload);
...
if (isset($_GET['edit'])) {
    $stmt = $db->prepare('SELECT * FROM student_answers WHERE student_answer_id = :id');
    $stmt->execute([':id' => (int) $_GET['edit']]);
    $row = $stmt->fetch();
    if ($row) { $form = array_merge($form, $row); }
}

$rows          = $db->query('SELECT * FROM student_answers ORDER BY student_answer_id DESC LIMIT 50')->fetchAll();
$attempts      = $db->query('SELECT student_exam_id, exam_id, student_id FROM student_exams ORDER BY student_exam_id DESC')->fetchAll();
$questions     = $db->query('SELECT question_id, topic, question_text FROM question_bank ORDER BY question_id DESC')->fetchAll();
$correctCount  = (int) $db->query('SELECT COUNT(*) FROM student_answers WHERE is_correct = 1')->fetchColumn();
$answeredCount = (int) $db->query('SELECT COUNT(*) FROM student_answers WHERE answered_at IS NOT NULL')->fetchColumn();
...
<div class="page">
    ...
    <div class="page-head">
        <div>
            ...
            <p>Answer rows per question per attempt — includes question snapshot JSON for audit.</p>
        </div>
        <div class="actions">
                        ...
                        <select name="question_id" required>
                            ...
                            <?php foreach ($questions as $question): ?>
                                <option value="<?= (int) $question['question_id'] ?>" <?= (string) old($form, 'question_id') === (string) $question['question_id'] ? 'selected' : '' ?>>
                                    #<?= (int) $question['question_id'] ?> · <?= e($question['topic']) ?> · <?= e(mb_strimwidth($question['question_text'], 0, 52, '…')) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($row['selected_option']): ?>
                                    <span class="badge manual"><?= e((string) $row['selected_option']) ?></span>
                                <?php else: ?>
