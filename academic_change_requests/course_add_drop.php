<?php
$acrActive = 'course_add_drop';
$pageTitle = 'Course Add/Drop Request';
require_once __DIR__ . '/_common.php';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $student_ids = isset($_POST['student_ids']) && is_array($_POST['student_ids']) ? array_map('intval', $_POST['student_ids']) : [];
    $add_course_ids = isset($_POST['add_course_ids']) && is_array($_POST['add_course_ids']) ? array_map('intval', $_POST['add_course_ids']) : [];
    ...
    } elseif (empty($add_course_ids) && empty($remove_course_ids)) {
        $error = "Please select courses to add or drop.";
    } else {
        $processed = 0;
        $fails = [];
        foreach ($student_ids as $id) {
            $r = acr_apply_courses($id, $add_course_ids, $remove_course_ids);
            if ($r['ok']) { $processed++; } else { $fails[] = $r['msg']; }
        }
        $success = "Applied course add/drop to $processed student(s).";
...
?>
    ...
    <div class="container-fluid">
        <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <h2><i class="fas fa-book-open"></i> Course Add/Drop Request</h2>
                <span class="text-muted small">Add or drop courses for admitted students.</span>
            </div>
        ...
        <form method="POST" id="changeForm">
            ...
            <div class="panel mt-3" style="border:1px dashed var(--border);">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <h5 class="m-0"><i class="fas fa-book-open"></i> Apply Course Add/Drop</h5>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="add_course_ids[]" class="form-select" multiple style="min-height:130px;">
                ...
                <div class="mt-3">
                    <button type="submit" class="btn btn-primary" onclick="return validateCourses()">
                        <i class="fas fa-check-circle"></i> Apply Course Add/Drop
                    </button>
                </div>
            </div>
            <?php endif; ?>
...
function validateCourses() {
    ...
    const add = document.querySelector('select[name="add_course_ids[]"]');
    const rem = document.querySelector('select[name="remove_course_ids[]"]');
    if ((!add || add.selectedOptions.length === 0) && (!rem || rem.selectedOptions.length === 0)) {
