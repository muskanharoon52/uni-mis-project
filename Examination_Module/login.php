...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = db()->prepare('SELECT * FROM examination_auth_users WHERE login_id = :login_id AND role = :role LIMIT 1');
        $stmt->execute([':login_id' => $loginId, ':role' => 'Examiner']);
        $user = $stmt->fetch();
    } catch (Throwable $exception) {
        $user = null;
    }

    $fallback = $config['demo_auth']['examiner'][$loginId] ?? null;
...
<head>
    ...
    <title>Sign In | Examination Portal</title>
    <meta name="description" content="Sign in to Examination Portal">
...
<div class="login-container" style="max-width: 960px; min-height: 560px; border-radius: var(--radius-lg); box-shadow: 0 20px 40px rgba(0,0,0,0.06); background: #fff;">
    <section class="login-hero" style="background: linear-gradient(135deg, #67e8f9 0%, #0891b2 40%, #0e7490 100%); padding: 48px; display: flex; flex-direction: column; justify-content: space-between; border: none; overflow: hidden; position: relative;">
        ...
        <div style="margin-top: auto; color: #fff;">
            <p style="opacity: 0.85; font-size: 0.95rem; margin-bottom: 8px; font-weight: 500;">Examination Portal</p>
            <h2 style="font-size: 1.8rem; font-weight: 700; line-height: 1.35; color: #fff; margin: 0; letter-spacing: -0.02em;">View student results, manage promotions, and track academic progress</h2>
        </div>
    </section>
    ...
    <aside class="login-panel" style="padding: 48px; display: flex; flex-direction: column; justify-content: center; background: #fff;">
        ...
        <h3 style="font-size: 1.7rem; font-weight: 700; letter-spacing: -0.02em; color: var(--text-strong); margin: 0 0 6px;">Sign in to EXM</h3>
        <p style="color: var(--muted); font-size: 0.88rem; margin: 0 0 24px; line-height: 1.5;">Access examination results, student promotions, and academic records.</p>
        ...
        <div class="login-footer" style="margin-top: 24px; padding-top: 20px; border-top: 1px solid var(--border);">
            <p style="color: var(--muted); font-size: 0.78rem; margin: 0;">Demo: <strong>6001</strong> / <strong>exam123</strong></p>
        </div>
    </aside>
</div>
