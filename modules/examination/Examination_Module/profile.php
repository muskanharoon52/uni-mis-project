        ...
        try {
            if ($userId > 0) {
                if (!empty($newPassword)) {
                    ...
                    $stmt = $db->prepare('UPDATE examination_auth_users SET display_name = :display_name, password_hash = :password_hash WHERE auth_id = :auth_id');
                    $stmt->execute([
                    ...
                    $stmt = $db->prepare('UPDATE examination_auth_users SET display_name = :display_name WHERE auth_id = :auth_id');
                    $stmt->execute([
                        ':display_name' => $displayName,
                        ':auth_id' => $userId
                    ]);
...
<div class="page">
    <div class="page-head">
        <div>
            <h2>My Profile</h2>
            <p>View and manage your account configurations inside the Examination ERP.</p>
        </div>
        <div class="actions">
            <a class="btn btn-ghost" href="index.php">&larr; Dashboard</a>
        </div>
    </div>
    ...
    <div class="grid-2">
            ...
            <form method="post">
                ...
                <div class="form-grid">
                    <div class="field" style="grid-column: 1 / -1;">
                        ...
                        <input type="text" name="display_name" required value="<?= e($user['display_name']) ?>" placeholder="e.g. Examiner Jane Doe">
                    </div>
                </div>
                ...
                <div class="form-grid">
                    <div class="field">
                        <label>Role</label>
                        <input type="text" disabled value="<?= e($user['role']) ?>" style="background-color: var(--panel-strong); border-color: var(--border);">
                    </div>
                    <div class="field">
        ...
        <div class="card">
            ...
            <div style="display:grid; gap:8px; font-size:.85rem; color:var(--text);">
                <div>Status: <span class="badge badge-active" style="text-transform:uppercase;"><?= e($user['status']) ?></span></div>
                <div>Account Type: <strong><?= $userId > 0 ? 'Database Backed' : 'Virtual Session User (Demo)' ?></strong></div>
                <div>Module: <strong>Examination Portal (EXM)</strong></div>
            </div>
        </div>
    </div>
</div>
