...
$columns = [];
if ($columns_query) { while($col = $columns_query->fetch_assoc()) { $columns[] = $col['Field']; } }
...
$programCourses = [];
if($program_id > 0) {
    $sql = "SELECT c.*, d.department_name, p.program_name FROM courses c LEFT JOIN departments d ON c.department_id = d.department_id LEFT JOIN programs p ON c.program_id = p.program_id WHERE c.program_id = $program_id ORDER BY c.course_code";
    $programCourses_result = $conn->query($sql);
    if ($programCourses_result && $programCourses_result->num_rows > 0) { $programCourses = $programCourses_result->fetch_all(MYSQLI_ASSOC); }
}
if($program_id == 0) {
    $sql = "SELECT c.*, d.department_name, p.program_name FROM courses c LEFT JOIN departments d ON c.department_id = d.department_id LEFT JOIN programs p ON c.program_id = p.program_id ORDER BY c.course_code";
    $allCourses_result = $conn->query($sql);
    if ($allCourses_result) { $programCourses = $allCourses_result->fetch_all(MYSQLI_ASSOC); }
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if($selected_program == 0 || $selected_semester == 0 || empty($selected_courses)) {
        ...
        foreach($selected_courses as $course_id) {
            ...
            $check_types = "ii";
            if($has_program_column) { $check_sql .= " AND program_id = ?"; $check_params[] = $selected_program; $check_types .= "i"; }

            $stmt = $conn->prepare($check_sql);
            if ($stmt === false) { $error = "Error preparing check query: " . $conn->error; break; }
            ...
            if($check_result->num_rows == 0) {
                ...
                $insert_types = "ii";
                if($has_program_column) { $insert_fields[] = "program_id"; $insert_values[] = $selected_program; $insert_types .= "i"; }
                if($has_assigned_date) { $insert_fields[] = "assigned_date"; $insert_values[] = date('Y-m-d H:i:s'); $insert_types .= "s"; }
                if($has_assigned_by) { $insert_fields[] = "assigned_by"; $insert_values[] = $_SESSION['user_id'] ?? 1; $insert_types .= "i"; }
                if($has_created_at) { $insert_fields[] = "created_at"; $insert_values[] = date('Y-m-d H:i:s'); $insert_types .= "s"; }

                $insert_sql = "INSERT INTO semester_courses (" . implode(", ", $insert_fields) . ") VALUES (" . implode(", ", array_fill(0, count($insert_fields), "?")) . ")";
                $insert_stmt = $conn->prepare($insert_sql);
                if ($insert_stmt === false) { $error = "Error preparing insert query: " . $conn->error; break; }
                $insert_stmt->bind_param($insert_types, ...$insert_values);
                if($insert_stmt->execute()) { $assigned_count++; }
                else { $success = false; $error = "Error assigning courses: " . $insert_stmt->error; break; }
                $insert_stmt->close();
        ...
        if($success && $assigned_count > 0) {
            ...
            if($already_assigned > 0) { $success_msg .= " ($already_assigned course(s) were already assigned)"; }
        } elseif($success && $assigned_count == 0 && $already_assigned > 0) { $error = "All selected courses are already assigned to this semester!"; }
    }
}
...
<form method="POST" action="" id="assignForm">
    <div class="form-container" style="margin-bottom:16px;">
        <div class="form-row">
            <div class="form-group">
                <label><i class="fas fa-graduation-cap"></i> Select Program</label>
                <select name="program_id" id="program_id" required>
                    <option value="0">-- Select Program --</option>
                    <?php foreach($programs as $program): ?>
                        <option value="<?= $program['id'] ?>" <?= ($program_id == $program['id']) ? 'selected' : '' ?>><?= htmlspecialchars($program['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="semester_id" id="semester_id" required>
                    ...
                    <?php foreach($semesters as $semester): ?>
                        <option value="<?= $semester['id'] ?>" <?= ($semester_id == $semester['id']) ? 'selected' : '' ?>><?= htmlspecialchars($semester['semester_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
    ...
    <?php if(!empty($programCourses)): ?>
        <div class="card">
            <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
                <h5><i class="fas fa-list"></i> Select Courses to Assign <span style="font-size:12px;font-weight:600;color:var(--muted);background:var(--bg);padding:2px 8px;border-radius:999px;margin-left:8px;"><?= count($programCourses) ?> courses found</span></h5>
            ...
            <div style="padding:0;">
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody>
                            <?php $i = 1; foreach($programCourses as $course): $assigned = false;
                                if($semester_id > 0 && $program_id > 0) {
                                    $check_sql = "SELECT * FROM semester_courses WHERE semester_id = ? AND course_id = ?"; $check_params = [$semester_id, $course['course_id']]; $check_types = "ii";
                                    if($has_program_column) { $check_sql .= " AND program_id = ?"; $check_params[] = $program_id; $check_types .= "i"; }
                                    $stmt = $conn->prepare($check_sql); if ($stmt !== false) { $stmt->bind_param($check_types, ...$check_params); $stmt->execute(); $check_result = $stmt->get_result(); $assigned = $check_result->num_rows > 0; $stmt->close(); }
                                }
                            ?>
                            <tr>
                                <td><input type="checkbox" name="courses[]" value="<?= $course['course_id'] ?>" class="course-checkbox" style="width:auto;" <?= $assigned ? 'disabled' : '' ?> <?= (in_array($course['course_id'], $preselected_course)) ? 'checked' : '' ?>></td>
                                <td><?= $i++ ?></td>
                                ...
                                <td><?= $course['credit_hours'] ?? 0 ?> Credits</td>
    ...
    <?php else: ?>
        <div class="empty-state">
            ...
            <p><?php if($program_id > 0): ?>No courses available for this program.<?php else: ?>Please select a program from the dropdown above to see available courses.<?php endif; ?></p>
            <?php if($program_id > 0): ?>
