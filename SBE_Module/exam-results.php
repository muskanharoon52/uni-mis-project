...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $stmt = $db->prepare('DELETE FROM sbe_exam_results WHERE exam_result_id = :id');
    ...
    if (!empty($_POST['exam_result_id'])) {
        ...
        $stmt = $db->prepare('UPDATE sbe_exam_results SET student_exam_id = :student_exam_id, exam_id = :exam_id, student_id = :student_id, obtained_marks = :obtained_marks, total_marks = :total_marks, percentage = :percentage, pass_fail_status = :pass_fail_status, rank_position = :rank_position, remarks = :remarks, status = :status, published_at = :published_at WHERE exam_result_id = :exam_result_id');
        $stmt->execute($payload);
        $_SESSION['message'] = 'Result updated successfully.';
    } else {
        $stmt = $db->prepare('INSERT INTO sbe_exam_results (student_exam_id, exam_id, student_id, obtained_marks, total_marks, percentage, pass_fail_status, rank_position, remarks, status, published_at) VALUES (:student_exam_id, :exam_id, :student_id, :obtained_marks, :total_marks, :percentage, :pass_fail_status, :rank_position, :remarks, :status, :published_at)');
        $stmt->execute($payload);
...
if (isset($_GET['edit'])) {
    $stmt = $db->prepare('SELECT * FROM sbe_exam_results WHERE exam_result_id = :id');
...
<div class="page">
    ...
    <div class="table-card page-section">
        ...
        <p class="small" style="margin:0 0 16px;">All result snapshots stored in <strong>university_mis</strong>.</p>
        <div class="table-wrapper">
            <table>
                <thead>
                    <tr>
                ...
                <?php else: ?>
                    <?php foreach ($rows as $row): ?>
                        <tr>
                            ...
                            <td><span class="badge badge-<?= e(strtolower($row['status'])) ?>"><?= e($row['status']) ?></span></td>
                            <td>
                                <div class="actions">
