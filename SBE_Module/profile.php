        ...
        try {
            if ($userId > 0) {
                if (!empty($newPassword)) {
                    ...
                    $stmt = $db->prepare('UPDATE sbe_auth_users SET display_name = :display_name, password_hash = :password_hash WHERE auth_id = :auth_id');
                    $stmt->execute([
                    ...
                    $stmt = $db->prepare('UPDATE sbe_auth_users SET display_name = :display_name WHERE auth_id = :auth_id');
                    $stmt->execute([
                        ':display_name' => $displayName,
                        ':auth_id' => $userId
                    ]);
    ...
    <div class="page-head">
        ...
        <div class="actions">
            <?php if ($user['role'] === 'Teacher'): ?>
                <a class="btn btn-ghost" href="teacher-home.php">&larr; Teacher Portal</a>
            <?php else: ?>
                <a class="btn btn-ghost" href="student-home.php">&larr; Student Portal</a>
            <?php endif; ?>
        </div>
    </div>
