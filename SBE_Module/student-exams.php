...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $stmt = $db->prepare('DELETE FROM sbe_student_exams WHERE student_exam_id = :id');
...
<div class="page">
                ...
                <div class="form-grid">
                        ...
                        <select name="schedule_id" required>
                            ...
                            <?php foreach ($schedules as $schedule): ?>
                                <option value="<?= (int) $schedule['schedule_id'] ?>" <?= (string) old($form, 'schedule_id') === (string) $schedule['schedule_id'] ? 'selected' : '' ?>>
                                    #<?= (int) $schedule['schedule_id'] ?> &middot; <?= e($schedule['exam_code']) ?> &middot; <?= e($schedule['section']) ?> &middot; <?= e($schedule['exam_date']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="field">
                        ...
                        <select name="status" required>
                            <?php foreach (['In Progress', 'Submitted', 'Auto Submitted', 'Expired', 'Cancelled'] as $status): ?>
                    ...
                    <div class="field">
                        ...
                        <select name="pass_fail_status">
                            <option value="">&mdash; Not yet &mdash;</option>
    ...
    <div class="table-card page-section">
        ...
        <p class="small" style="margin:0 0 16px;">All student attempts stored in <strong>university_mis</strong>.</p>
        <div class="table-wrapper">
            <table>
                <thead>
                    <tr>
